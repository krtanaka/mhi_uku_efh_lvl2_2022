...
for (k in 1:length(n_knots)) {
  ...
  map = map %>% 
    mutate(island_group = case_when(lat > 21.73143 & lat < 22.29684 & lon > -160.3023 & lon < -159.2321 ~ "Niihau-Kauai",
                                    lat > 21.19259 & lat < 21.75448 & lon > -158.3432 & lon < -157.5982 ~ "Oahu",
                                    lat > 20.45111 & lat < 21.26553 & lon > -157.3642 & lon < -155.9242 ~ "Molokai-Maui-Lanai-Kahoolawe",
                                    lat > 18.86465 & lat < 20.32120 & lon > -156.1109 & lon < -154.7542 ~ "Hawaii",))
  
  map$island_group = factor(map$island_group, levels=c('Niihau-Kauai','Oahu','Molokai-Maui-Lanai-Kahoolawe','Hawaii'))
  ...
  (fig4aa = map %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Niihau-Kauai", subtitle = "Max. estimated density per 100 sq.m = 3.6") +
  ...
  (fig4ab = map %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Oahu", subtitle = "Max. estimated density per 100 sq.m = 1.01") +
  ...
  (fig4ac = map %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Molokai-Maui-Lanai-Kahoolawe", subtitle = "Max. estimated density per 100 sq.m = 7.27") +
      theme(legend.justification = c(0, 0), 
            legend.position = c(0, 0),
            legend.key = element_rect(colour = NA, fill = NA),
            legend.text = element_text(color = "white", size = 12),
  ...
  (fig4ad = map %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Hawaii", subtitle = "Max. est. density per 100 sq.m = 2.11") +
  ...
  (fig4ba = slope %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Niihau-Kauai", subtitle = "Median change rate = -0.054 year-1") +
      theme(legend.justification = c(0, 1), 
            legend.position = c(0, 1.05),
            legend.key = element_rect(colour = NA, fill = NA),
            legend.text = element_text(color = "white", size = 12),
  ...
  (fig4bb = slope %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Oahu", subtitle = "Median change rate = -0.057 year-1") +
      theme(legend.justification = c(0, 0), 
  ...
  (fig4bc = slope %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Molokai-Maui-Lanai-Kahoolawe", subtitle = "Median change rate = -0.085 year-1") +
      theme(legend.justification = c(0, 0), 
            legend.position = c(0, 0.01),
            legend.key = element_rect(colour = NA, fill = NA),
            legend.text = element_text(color = "white", size = 12),
  ...
  (fig4bd = slope %>%
      ...
      ylab("") + xlab("") + 
      theme_minimal() + 
      ggtitle("Hawaii", subtitle = "Median change rate: -0.073 year-1") +
      theme(legend.justification = c(1,1), 
