...
for (l in 1:3) {
  ...
  if (l == 3) {
    ...
    # r <- raster(ncol = 18000, nrow = 9000)
    # r <- raster(ncol = 14400, nrow = 7200)
    # r <- raster(ncol = 7200, nrow = 3600)
    r <- raster(ncol = 3600, nrow = 1800)
    # r <- raster(ncol = 1800, nrow = 900)
...
rm(df, Final.b, l, d_i, lat_range, lon_range, i, plot_list, quantiles, uku_cpue, p_i, r)
...
for (i in 1:length(isl_groups)) {
  ...
  d_1 = df_all %>% 
    subset(island_group == isl_groups[i] & lvl == "cpue") %>% 
    mutate(lon = round(lon, 2),
           lat = round(lat, 2)) %>%
    group_by(lon, lat, island_group) %>%
  ...
  d_2 = df_all %>% 
    subset(island_group == isl_groups[i] & lvl == "EFH level 1") %>% 
    mutate(lon = round(lon, 2),
           lat = round(lat, 2)) %>%
    group_by(lon, lat, island_group) %>%
    summarise(percentile = mean(percentile, na.rm = T)) %>% 
  ...
  d_3 = df_all %>% 
    subset(island_group == isl_groups[i] & lvl == "EFH level 2") %>% 
    mutate(lon = round(lon, 2),
           lat = round(lat, 2)) %>%
    group_by(lon, lat, island_group) %>%
    summarise(percentile = mean(percentile, na.rm = T)) %>% 
  ...
  d_i = rbind(d_1, d_2, d_3) %>% 
    group_by(lon, lat) %>% 
    mutate(percentile_mean = mean(percentile))
  ...
  if (i == 3) {
    ...
    p_i <- d_i %>% 
      filter(lvl != "cpue") %>%
      ggplot(aes(lon, lat, fill = percentile_mean)) + 
...
(plot_list[[1]] + plot_list[[2]]) / (plot_list[[3]] + plot_list[[4]])
rm(isl_groups, plot_list, i, d_1, d_2, d_3, d_i, lat_range, lon_range, p_i)
ggsave(last_plot(), filename = "outputs/percentile_cpue_efh.png", height = 8, width = 11)

options = c("Op2", "Op3", "Op5")
...
for (o in 1:length(options)) {
  ...
  if (o == 3) {
    ...
    df_i = df_i %>% 
      filter(lvl != "cpue") %>% 
      group_by(lon, lat, island_group) %>%
      summarise(percentile = mean(percentile_mean))
  ...
  if (o == 2) ggsave(last_plot(), filename = "outputs/option_3_percentile_efh.png", height = 8, width = 22)
  if (o == 3) ggsave(last_plot(), filename = "outputs/option_5_percentile_efh.png", height = 8, width = 22)
  ...
  if (o == 1) ggsave(last_plot(), filename = "outputs/option_2_percentile_efh_just_efh.png", height = 8, width = 22)
  if (o == 2) ggsave(last_plot(), filename = "outputs/option_3_percentile_efh_just_efh.png", height = 8, width = 22)
  if (o == 3) ggsave(last_plot(), filename = "outputs/option_5_percentile_efh_just_efh.png", height = 8, width = 22)
